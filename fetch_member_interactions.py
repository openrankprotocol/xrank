...
def extract_post_data(post_data):
    ...
    try:
        ...
        # Primary check: community_results.result structure (most reliable)
        if "community_results" in tweet_data:
            community_data = tweet_data["community_results"]
            if isinstance(community_data, dict) and "result" in community_data:
                result = community_data["result"]
                if isinstance(result, dict):
                    # Extract community ID (can be id_str or rest_id)
                    community_id = result.get("id_str") or result.get("rest_id")
                    community_name = result.get("name")
                    if community_id:
                        is_community_post = True

        # Fallback: Look for community information in other places
        if not community_id and "community" in tweet_data:
            community_info = tweet_data["community"]
            if isinstance(community_info, dict):
                community_id = community_info.get("id_str") or community_info.get(
                    "rest_id"
                )
        ...
        # Additional fallback for communities or community_note keys
        if not community_id:
            for key in ["communities", "community_note"]:
                if key in tweet_data:
                    community_data = tweet_data[key]
                    if isinstance(community_data, dict):
                        if "result" in community_data:
                            result = community_data["result"]
                            community_id = result.get("id_str") or result.get("rest_id")
                            community_name = result.get("name")
                            if community_id:
                            ...
                            community_id = community_data.get(
                                "id_str"
                            ) or community_data.get("rest_id")
        ...
        # Determine if post is a retweet based on text pattern
        # A retweet starts with "RT @username: " and is not a quote
        is_retweet = False
        rt_username = None
        if not is_quote and post_text.startswith("RT @"):
            # Extract username from RT @username: pattern
            rt_match = re.match(r"^RT @(\w+):\s", post_text)
            if rt_match:
                is_retweet = True
                rt_username = rt_match.group(1)
        ...
        # If we detected a retweet from text pattern, set the original creator username
        if is_retweet and rt_username:
            extracted_data["original_post_creator_username"] = rt_username
        ...
        if extracted_data["is_retweet"] and "retweeted_status_result" in tweet_data:
            ...
            if "legacy" in rt_result:
                ...
                rt_user = (
                    rt_core.get("user_results", {}).get("result", {}).get("legacy", {})
                )

                # Set original post creator info in main data
                extracted_data["original_post_creator_id"] = rt_legacy.get(
                    "user_id_str", ""
                )
                extracted_data["original_post_creator_username"] = rt_user.get(
                    "screen_name", ""
                )
        ...
        if extracted_data["is_quote"] and "quoted_status_result" in tweet_data:
            ...
            if "legacy" in qt_result:
                ...
                qt_user = (
                    qt_core.get("user_results", {}).get("result", {}).get("legacy", {})
                )

                # Set original post creator info in main data
                extracted_data["original_post_creator_id"] = qt_legacy.get(
                    "user_id_str", ""
                )
                extracted_data["original_post_creator_username"] = qt_user.get(
                    "screen_name", ""
                )
                            ...
                            for entry in instruction["entries"]:
                                if "tweet-" in entry.get("entryId", ""):
                                    ...
                                    if (
                                        ...
                                        content_data = entry_content["itemContent"][
                                            "tweet_results"
                                        ].get("result", {})
                                        ...
                                        if is_post_within_days(content_data, days_back):
                                            ...
                                            print(
                                                f"    Reached content outside date range for @{username}"
                                            )
                                            return content
        except Exception as e:
...
def get_user_replies(username, user_id, days_back, max_replies=1000):
    ...
    return get_user_content(
        username, user_id, "/user-replies-v2", days_back, max_replies
    )
